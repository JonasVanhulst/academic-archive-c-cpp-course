...
int main(void) {
  ...
  printf("%s\n", (rayPlaneIntersection(plane, ray) == FALSE)
                     ? "No ray plane intersection"
                     : "Ray plane intersection");
  printf("%s\n", (rayTrianlgeIntersection(triangle, ray) == FALSE)
...
/*
  The dot product of two points (e.g. p1(x1, y1, z1) and p2(x2, y2, z2)) is the
  sum of the product of the x, y, z coordinates. x1*x2 + y1*y2 + z1*z2
*/
float dotProduct(struct point3D p1, struct point3D p2) {
...
/*
 The cross product of two points (e.g. p1(x1, y1, z1) and p2(x2, y2, z2))
 results in a new point (e.g. p(x, y, z)) and can be calculated as: x = y1*z2 -
 z1*y2 y = z1*x2 - x1*z2 z = x1*y2 - y1*x2
*/
struct point3D crossProduct(struct point3D p1, struct point3D p2) {
