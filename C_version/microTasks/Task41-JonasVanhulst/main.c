...
void splitTriangle(struct triangle* triangle, struct triangle** trianglePartOne,
                   struct triangle** trianglePartTwo);
...
/* Break Down the given triangle into smaller triangles of which the surface
area is not bigger than the given maxSurfaceArea. To do this, make use of a
linked-list LIFO buffer for the todo-list and the returning solution-list. Start
by creating a pointer to the final solution and one to the todo-list which
points to the given triangle. As long as there is work todo in the todo-list,
take the next triangle :
- if its area is larger than the threshold, split the triangle with the function
splitTriangle(), add the two returned pointers onto the todo-list (LIFO-style :
push one before two) and free the memory of the processed triangle.
- if its area is not bigger, add that triangle onto the solution-list
(LIFO-style : push triangle).
*/
struct triangle* breakDownTriangle(struct triangle* triangle,
                                   float maxSurfaceArea) {
  struct triangle* finalSolution = NULL;
  struct triangle* todoList = triangle;

  while (todoList != NULL) {
    struct triangle* currentTriangle = todoList;
    todoList = todoList->next;

    if (surfaceArea(currentTriangle) > maxSurfaceArea) {
      struct triangle* trianglePartOne;
      struct triangle* trianglePartTwo;
      ...
      todoList->next = trianglePartOne;
      free(currentTriangle);
    } else {
      currentTriangle->next = finalSolution;
      finalSolution = currentTriangle;
    }
  }
...
float surfaceArea(struct triangle* triangle) {
  ...
  float surface =
      0.5 * sqrt(acCrossAb.x * acCrossAb.x + acCrossAb.y * acCrossAb.y +
                 acCrossAb.z * acCrossAb.z);

  // printf( "Cross product = (%f, %f, %f)\n", acCrossAb.x, acCrossAb.y,
  // acCrossAb.z ); printf( "Triangle surface = %f\n", surface );
  return surface;
}
...
void printResults(struct triangle* triangles) {
  ...
  while (triangle != NULL) {
    printf("%d) (%f, %f, %f) (%f, %f, %f) (%f, %f, %f)\n", counter + 1,
           triangle->p1.x, triangle->p1.y, triangle->p1.z, triangle->p2.x,
           triangle->p2.y, triangle->p2.z, triangle->p3.x, triangle->p3.y,
           triangle->p3.z);
    counter++;
