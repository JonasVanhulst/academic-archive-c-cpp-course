...
float gemiddeldeTemp(struct statistiek *data) {
    ...
    fprintf(fileSom, "De data van %d/%d/%d is : %f\n", data->lijst->datum.dag, data->lijst->datum.maand, data->lijst->datum.jaar, gemiddelde);
    fclose(fileSom);
...
float somNeerslag(struct statistiek *data) {
    ...
    FILE *fileSom = fopen("samenvatting.log", "a");
    fprintf(fileSom, "De data van %d/%d/%d is : %f\n", data->lijst->datum.dag, data->lijst->datum.maand, data->lijst->datum.jaar, som);
    fclose(fileSom);
...
void addValue(struct statistiek *data, float value, char list[20]) {
    ...
    fprintf(filePointer, "De data van %d/%d/%d is : %f\n", data->lijst->datum.dag, data->lijst->datum.maand, data->lijst->datum.jaar, value);
    fclose(filePointer);
}
