...
//
//Schrijf een programma waarbij je een struct declareert met de naam geolocatie.
// Deze struct bevat het land, de stad, lattitude en longitude.
// Zorg ervoor dat een gebruiker tot 10 geolocaties kan ingeven
// en deze geolocaties gesorteerd kunnen worden op basis van het land, de stad, lattitude en longitude.
// Declareer en defineer hiervoor de volgende functies:
//
//
...
void print_geolocaties(int total, struct geolocatie geolocaties[]) {
    ...
    for (int i = 0; i < MAX_NUM_GEOLOCATIONS; i++) {
        printf("%d. Country: %s, City: %s, Latitude: %s, Longitude: %s\n", i + 1, geolocaties[i].land, geolocaties[i].stad, geolocaties[i].lattitude, geolocaties[i].longitude);
    }
}
