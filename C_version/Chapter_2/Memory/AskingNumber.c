...
//
// Schrijf de functie vraagGetal() zo dat je een getal kan meegeven als een
// string (bijv. "42", "42.069", "3 + 2i") en parsed uit de string (i.e. bevat
//een 'i' dan is het COMPLEX, bevat een '.' dan is het KOMMA en anders GEHEEL).
// De functie reserveert dan geheugen op de heap voor de void pointer waarin de
// data (bijv. 42, 42.069, struct complex{3, 2}) wordt opgeslagen. Deze void
// pointer wordt bewaard binnen een struct getal samen met het type (bijv.
// GEHEEL, KOMMA, COMPLEX). Deze struct getal wordt tot slot gereturned door de
// functie vraagGetal(). Geef vervolgens twee getallen van het type struct getal
// mee met de functie som(). In de functie som worden deze twee getallen bij
// elkaar opgeteld en het resultaat geprint.
//
//```c
//
...
struct getal {
  enum { GEHEEL, KOMMA, COMPLEX } type;
  void* data;
};
