...
int initialization() {
  ...
  while (internet_address_result_iterator != NULL) {
    ...
    if (internet_socket == -1) {
      ...
      // Step 1.3
      int connect_return =
          connect(internet_socket, internet_address_result_iterator->ai_addr,
...
void execution(int internet_socket) {
  char bewerking[50];
  printf("Geef een bewerking (getal, bewerking, getal): ");
  fgets(bewerking, sizeof(bewerking), stdin);

  // Step 2.1
  int number_of_bytes_send = 0;
  number_of_bytes_send = send(internet_socket, bewerking, sizeof(bewerking), 0);
