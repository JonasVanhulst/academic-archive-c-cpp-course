//
// Created by Vanhuljo on 28/03/2023.
//
// ## Oefening 1
// Herschrijf oefening 1 van week 6.
// De data hoeft niet langer meegegeven te worden via argumenten naar de main.
// Wel dien je gebruik te maken van een linked list structuur (die werkt als een
// FIFO buffer) in plaats van een array in de struct statistiek. Hierin hou je
// de data van de laatste 2 maanden bij. Deze data wordt toegevoegd aan de
// linked list d.m.v. de AddValue() functie(s). Zodra er data wordt toevoegd die
// tot een nieuwe maand behoort, en er al twee maanden bestaan, dan wordt de
// oudste maand verwijderd. Voordat je deze data verwijdert bereken je eerst de
// cumul en het gemiddelde van de temperatuur en neerslag van die maand.
// EXTRA: Sla deze data op in een nieuwe linked list.
//
//
#include <stdint.h>
#include <stdio.h>
...
void addValueWithDate(struct statistiek* data, float value, struct date datum) {
  if (data->lijst == NULL) {
    ...
    pointer->next->waarde = value;
    pointer->next->datum.dag = datum.dag;  // Aantal dagen == toe te voegen
                                           // datum
