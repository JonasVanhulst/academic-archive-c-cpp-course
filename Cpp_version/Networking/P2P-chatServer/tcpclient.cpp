 ...
 *
 * 2. Creates a new Tcpserver instance for... well the tcpserver functions of
 * the p2p chat
 *
...
/*
 * 1. Receive a new connection from the QTcpServer and store it in a QTcpSocket
 * object
 *
 ...
 *
 * 4. If there is new data available from the new socket, the readFromAll slot
 * will handle it
 *
...
/*
 * 1. Go through the entire list of sockets and check if there are newly
 * received messages
 *
...
void TcpClient::firstConnect(std::string firstIp, int firstPort) {
   ...
   *
   * 2. Connect to the "host" with the information that was provided as
   * arguments to main
   *
   * 3. Write a "Hello there!" message to that connection
   *
   * 4. Wait for that message to be written (1 sec) and wait for the message to
   * be send (1 sec)
   *
   */
  ...
  if (firstSocket->waitForConnected()) {
    /* 1. Connect to readyRead "signal" of the firstSocket object, slot
     * readFromAll
     *
     * 2. Add the host socket to the list and emit the newConnection signal
     *    with the socket as the parameter to notify other parts of the program
    ...
    // Receive the message from the host, this should contain other IP addresses
    // and ports
    ...
    while (getline(ss, token)) {
      // Create a "temp" string and a stringstream token to keep track of the
       ...
       *
       * 3. Append the socket to the socketlist and send a readyRead signal with
       * the socket as the parameter
       */
      if (socket->state() == QAbstractSocket::ConnectedState) {
        std::cout << "Connected to " << newIP << ":" << newPort
    ...
    qDebug() << "Failed to connect to: " << firstIp.c_str() << ":"
             << firstSocket;
  }
}
 ...
 *
 * 3. If they are then convert the IP address to a string format and add the
 * port information to that string. Do this for every connection and add them
 * together as a complete string
 *
