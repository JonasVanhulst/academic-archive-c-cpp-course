...
// This class inherits from the QObject class, this creates the possibility to
// use signal-slot communication in Qt
class TcpClient : public QObject {
 ...
 public:
  ...
  QString getIP(void) {
    if (!m_sockets.empty()) {
      return m_sockets.first()->localAddress().toString();
    } else {
      return "";
    }
  };
 ...
 signals:
  // The different signal "channels" to send data and notify the right slot
