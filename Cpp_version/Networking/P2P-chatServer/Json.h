 ...
 *
 * 2. Create a string containing the ID, Timestamp and the message parsed from
 * the Json string
 *
...
QString JSONtoMessage(QString Json) {
  ...
  if (Parser.getId().isEmpty() || Parser.getTimestamp().isEmpty() ||
      Parser.getMessage().isEmpty()) {
    return "";
  }

  output =
      Parser.getId() + " " + Parser.getTimestamp() + "\n" + Parser.getMessage();
  return output;
}
#endif  // JSON_H
