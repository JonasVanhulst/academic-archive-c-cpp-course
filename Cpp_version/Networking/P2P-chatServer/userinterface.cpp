...
Userinterface::Userinterface(TcpClient* client) : Client(client) {
  ...
  QObject::connect(
      ...
      [client, debugTextEdit](QTcpSocket* socket) {
        qDebug() << "New connection from: " << socket->peerAddress().toString();
        debugTextEdit->appendPlainText("New connection from: " +
                                       socket->peerAddress().toString());
        QObject::connect(socket, &QTcpSocket::readyRead, client,
                         &TcpClient::readFromAll);
      });

  QObject::connect(client, &TcpClient::newMessageReceived,
                   [receivedTextEdit](QString message) {
                     qDebug() << "New message received: " << message;
                     receivedTextEdit->appendPlainText(JSONtoMessage(message));
                   });

  auto messageProcessingFunc = [this, debugTextEdit, receivedTextEdit,
                                inputLineEdit] {
    ...
    // intercept for json
    message = createJSON(Client->getNickName(), Client->getIP(),
  ...
  QObject::connect(inputLineEdit, &QLineEdit::returnPressed,
