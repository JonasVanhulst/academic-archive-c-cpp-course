...
/*
 * This is the constructor, it will initialize the id, ip, port, timestamp and
 * message to empty strings in the beginning
 *
 * 1. Parse the Json string to a Json document and store it in the "jsonDoc"
 * variable
 *
 * 2. If the json document isn't NULL and represents a JSON object then proceed
 * with the parsing
 *
 * 3. Extract (convert) the Json document to a Json object
 *
 * 4. Now it will extract and stort the id, ip, port, timestamp and the message
